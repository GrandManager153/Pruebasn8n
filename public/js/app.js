...
function openKpiModal(label, value) {
    ...
    setKpiModalContent({
        ...
        definition: simplifyKpiText((isForecastCard && ft?.explanation_es) ? ft.explanation_es : (explain?.definition || PLAIN_DEFAULTS.definition)),
        interpretation: simplifyKpiText(explain?.interpretation || PLAIN_DEFAULTS.interpretation),
        source: humanizeSource(explain?.source || 'CRM integrado vía n8n — operations'),
    });
}
...
function renderLittlesLawCards(ops) {
    ...
    const cards = [
        ...
        { label: '¿Alcanza el personal?', value: pressureLabel, sub: ll.staffing_gap_tomorrow > 0 ? `${gapLabel}: +${ll.staffing_gap_tomorrow} (faltan atenciones)` : 'Cobertura suficiente', color: ll.staffing_pressure === 'critical' ? 'red' : ll.staffing_pressure === 'pressure' ? 'gold' : 'green', animate: false },
...
const OPERATIONS_HIDDEN_KPI_LABELS = new Set([
    // Etiquetas técnicas (legado)
    'Tasa de llegada (λ)',
    'Tiempo de servicio (W)',
    'Cola estimada (L)',
    'Utilización',
    'Presión staffing',
    // Etiquetas en lenguaje claro (equivalentes)
    'Llegada de leads por hora',
    'Tiempo medio por contacto',
    'Personas en espera (estimado)',
    'Carga del equipo',
    '¿Alcanza el personal?',
