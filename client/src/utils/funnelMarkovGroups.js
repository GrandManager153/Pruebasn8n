export function isResidualMarkovState(rawState) {
  const raw = String(rawState || '').trim();
  return raw.toLowerCase() === 'abierto';
}

export function filterVisibleMarkovRows(rows) {
...
export function formatMarkovStateLabel(rawState, cleanFn) {
  const raw = String(rawState || '').trim();
  if (!raw) return '—';
...
export const MARKOV_GROUP_LABELS = {
...
export function resolveMarkovGroup(rawState, cleanedState) {
...
export function getMarkovGroupOptions(rows) {
...
export function filterMarkovByGroup(rows, groupId) {
